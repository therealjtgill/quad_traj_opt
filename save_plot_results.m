%% Plotting function
function plot_results(X,U,T,suffix)
    N = length(U);
    tX = linspace(0,T,N+1);
    tU = linspace(0,T,N);
    ...
    exportgraphics(gcf, 'motor_input' + suffix + '.png', 'Resolution', 300);
    ...
    for k = 1:1:(N + 1)
        ...
        exportgraphics(gcf, 'drone_pos_' + string(k) + suffix + '.png', 'Resolution', 300);
